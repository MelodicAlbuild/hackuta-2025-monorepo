...
export default function LiveSchedule({
  ...
  return (
    <div
      className="p-4 space-y-12 scrollbar-hide w-[40%]"
...
function isMobileWindowSize(): boolean {
...
function DayTimeline({
  ...
  return (
    <div>
      ...
      <div className="flex">
        {/* Time Gutter */}
        <div className="w-20 text-right pr-4 text-base text-white flex-shrink-0">
          {hours.map((hour) => (
            <div
              key={hour}
            ...
            >
              ...
              <span className="text-lg">{hour < 12 ? ' AM' : ' PM'}</span>
            </div>
          ))}
        </div>

        {/* Event Container */}
        <div className="flex-1 rounded-lg relative border-l border-gray-200">
          {/* Hour Lines */}
          {hours.map((hour) => (
            <div
              key={hour}
...
function EventCard({ event }: { event: ProcessedEvent }) {
  ...
  return (
    <Card
      className={`absolute p-2 rounded-lg text-base overflow-hidden border-l-4 ${style}`}
      style={{
        top: `${event.top}px`,
      ...
      <CardContent className="p-1 translate-y-[-10px]">
        {isMobileWindowSize() ? (
          <div className="grid grid-cols-1">
            <p className="font-bold text-md md:text-2xl">{event.title}</p>
            ...
            {event.location && (
              <p className="text-muted-foreground mt-1 truncate md:text-md text-sm">
                {event.location}
              </p>
            )}
          </div>
