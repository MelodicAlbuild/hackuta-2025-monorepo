...
// Use the shared browser client so we share auth/session correctly
...
export const WebSocketProvider = ({ children }: WebSocketProviderProps) => {
  const supabase = useRef<ReturnType<
    typeof createSupabaseBrowserClient
  > | null>(null);
  if (supabase.current === null) {
    supabase.current = createSupabaseBrowserClient();
  }
  ...
  const listeners = useRef<Map<string, Set<(msg: WsMsg) => void>>>(new Map());
  ...
  useEffect(() => {
    ...
    // Reconnect when auth state changes (sign-in, token refresh), and close on sign-out
    const { data: authListener } = supabase.current!.auth.onAuthStateChange(
      async (event) => {
        if (event === 'SIGNED_OUT') {
  ...
  const onChannelMessage = useCallback(
    (
      ...
      if (!listeners.current.has(prefix))
        listeners.current.set(prefix, new Set());
      listeners.current.get(prefix)!.add(handler);
    },
    [],
  );

  const offChannelMessage = useCallback(
    (
      prefix: string,
      handler: (message: {
        channel: string;
